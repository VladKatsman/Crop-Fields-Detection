...
class FlatBlock(nn.Module):  # ask about why it starts with 1024 filters ResnetBlocks
...
class MultiScaleResUNet(nn.Module):
    ...
    def forward(self, pyd):
        # If input is an image then generate image pyramid
        if not isinstance(pyd, (list, tuple)):
            pyd = self.create_pyramid(pyd)
