...
from field_boundaries.utils.losses import ComboLoss
...
from field_boundaries.utils import seg_transforms


def main(exp_dir='/data/experiments', train_dir=None, val_dir=None, workers=4, iterations=None, epochs=90,
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=batch_size, sampler=sampler, shuffle=not sampler,
    ...
    criterion = ComboLoss(weights={"focal": 1, "dice": 1}, channel_weights=[0.57, 0.134, 1.],
                          channel_losses=[["focal", "dice"], ["focal", "dice"], ["focal", "dice"]]
                          ).to(device)

    # define running metrics
    # running_metrics_val = seg_utils.RunningScore(train_dataset.n_classes)
...
def validate(val_loader, model, criterion, epoch, epochs, device, logger, running_metrics_val):
    ...
    if logger:
        ...
        # mask = dense_crf(images, output)
        gt = targets * 100
        pred = pred * 100
        mask = pred.view(pred.shape[0], 1, pred.shape[1], pred.shape[2]).repeat(1, 3, 1, 1).float()
        gt = gt.view(gt.shape[0], 1, gt.shape[1], gt.shape[2]).repeat(1, 3, 1, 1).float()
        grid = seg_utils.make_grid(images, gt.float(), mask, min(8, val_loader.batch_size))
...
if __name__ == "__main__":
    ...
    main(args.exp_dir, args.train, args.val, workers=args.workers, iterations=args.iterations, epochs=args.epochs,
         ...
         pretrained=args.pretrained, cudnn_benchmark=args.cudnn_benchmark,
